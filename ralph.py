...
class ClaudeCodeRalph:
    ...
    STOP_CONDITIONS = {
        "tests_pass": lambda: subprocess.run(
            [sys.executable, "-m", "pytest", "tests/test_api.py", "tests/test_api_integration.py", "-v", "--timeout=30", "-m", "not slow"], capture_output=True
        ).returncode == 0,
        ...
        "api_starts": lambda: subprocess.run(
            [sys.executable, "-c", "from swim_ai_reflex.backend.api.main import api_app"], 
            capture_output=True
        ).returncode == 0,
    }
...
def main():
    ...
    parser.add_argument(
        "--task",
        type=str,
        help="Description of the task to complete"
    )
    ...
    parser.add_argument(
        "--run-tests",
        action="store_true",
        help="Run the test-passing loop"
    )
    
    parser.add_argument(
        "--run-build",
        action="store_true",
        help="Run the build-fixing loop"
    )
