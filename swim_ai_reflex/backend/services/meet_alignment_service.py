...
def align_meet_data(seton_df: pd.DataFrame, opponent_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, dict]:
    """
    ...
    opponent_team_col = opponent_df['team'].iloc[0] if not opponent_df.empty and 'team' in opponent_df.columns else "Opponent"
    
    # Helper: Detect flexible 'opponent' column name
    def find_opponent_col(df):
        candidates = ['opponent', 'opponent team', 'opponent_team', 'visitor', 'visitor team', 'vs', 'vs team']
    ...
    logger.info(f"Meet Alignment: Seton team, Opponent team column = '{opponent_team_col}'")
    ...
    if 'opponent' in seton_df.columns and 'opponent' in opponent_df.columns:
        ...
        seton_opponent_values = seton_df['opponent'].dropna().unique().tolist() if 'opponent' in seton_df.columns else []
        opponent_opponent_values = opponent_df['opponent'].dropna().unique().tolist() if 'opponent' in opponent_df.columns else []
        
        logger.info(f"  Seton's 'opponent' column values: {seton_opponent_values[:5]}...")
        logger.info(f"  Opponent's 'opponent' column values: {opponent_opponent_values[:5]}...")
        ...
        seton_aligned = seton_df[
            (seton_df['team'].str.lower().str.contains('seton', na=False))  # Must be Seton team
        ].copy()
        
        # Find Opponent entries where opponent is Seton (Strict Check)
        opponent_aligned = opponent_df[
            (opponent_df['opponent'].str.lower().str.contains('seton', na=False))  # Their opponent says Seton
        ].copy()
        ...
        if opponent_aligned.empty:
            ...
            seton_opponents = seton_df['opponent'].dropna().astype(str).str.lower().unique()
            ...
            if match_found:
                logger.info(f"  One-sided match confirmed: Seton is swimming against '{opponent_team_col}'")
                opponent_aligned = opponent_df.copy() # Assume entire file is the meet roster for opponent
        
        # If both have data, we have a match!
        if not seton_aligned.empty and not opponent_aligned.empty:
            # Filter to common events
            seton_events = set(seton_aligned['event'].unique()) if 'event' in seton_aligned.columns else set()
            opponent_events = set(opponent_aligned['event'].unique()) if 'event' in opponent_aligned.columns else set()
            ...
            if common_events:
                ...
                alignment_info['removed_opponent'] = len(opponent_df) - len(opponent_aligned)
                alignment_info['common_events'] = sorted(list(common_events))
                alignment_info['alignment_method'] = 'opponent_column'
                
                logger.info(f"✓ Meet alignment (opponent column): {alignment_info['common_meet']}")
                logger.info(f"  Seton: {len(seton_df)} → {len(seton_aligned)} entries ({alignment_info['removed_seton']} removed)")
                logger.info(f"  Opponent: {len(opponent_df)} → {len(opponent_aligned)} entries ({alignment_info['removed_opponent']} removed)")
    ...
    seton_events = set(seton_df['event'].unique()) if 'event' in seton_df.columns else set()
    opponent_events = set(opponent_df['event'].unique()) if 'event' in opponent_df.columns else set()
    common_events = seton_events & opponent_events
    ...
    if common_events:
        ...
        if overlap_ratio > 0.3:
            ...
            logger.info(f"✓ Meet alignment (event overlap): {alignment_info['common_meet']}")
    ...
    logger.warning("❌ Strict Meet Alignment Failed. Could not match 'Opponent' columns and Event Overlap was insufficient.")
...
def validate_alignment(seton_df: pd.DataFrame, opponent_df: pd.DataFrame) -> dict:
    ...
    ratio = max(seton_count, opponent_count) / min(seton_count, opponent_count) if min(seton_count, opponent_count) > 0 else 0
