...
class ScoreValidationService(BaseService):
    ...
    def validate_scores(
        ...
        if seton_score > self.ERROR_THRESHOLD or opponent_score > self.ERROR_THRESHOLD:
            ...
            recommendations.append('Verify max_scorers_per_team rule is enforced (should be 3)')
            
        elif seton_score > self.WARNING_THRESHOLD or opponent_score > self.WARNING_THRESHOLD:
            warnings.append({
                'type': 'HIGH_SCORE',
                'severity': 'WARNING',
        ...
        if not roster_df.empty:
            ...
            if duplicate_check['has_duplicates']:
                ...
                recommendations.append('Use deduplication: df.drop_duplicates(subset=["swimmer", "event"])')
        
        # Check 3: Entries per event
        if not scored_df.empty and 'event' in scored_df.columns and 'team' in scored_df.columns:
        ...
        for (swimmer, event), group in duplicates.groupby(['swimmer', 'event']):
            if len(group) > 1:
                examples.append({
                    ...
                    'event': event,
                    'count': len(group),
                    'times': group['time'].tolist() if 'time' in group.columns else []
                })
    ...
    def detect_data_issues(self, df: pd.DataFrame) -> Dict[str, any]:
        ...
        if not duplicates.empty:
            dup_count = len(duplicates) - duplicates.drop_duplicates(subset=['swimmer', 'event']).shape[0]
            issues.append({
                'type': 'DUPLICATES',
                'severity': 'error',
        ...
        if 'time' in df.columns:
            invalid_times = df[(df['time'] <= 0) | (df['time'] > 600)]  # 0 or >10 minutes
        ...
        if 'swimmer' in df.columns:
            empty_names = df[df['swimmer'].isna() | (df['swimmer'].astype(str).str.strip() == '')]
    ...
    def auto_fix_roster(self, df: pd.DataFrame) -> Dict[str, any]:
        ...
        if 'swimmer' in fixed_df.columns:
            empty_mask = fixed_df['swimmer'].isna() | (fixed_df['swimmer'].astype(str).str.strip() == '')
            if empty_mask.any():
        ...
        if 'swimmer' in fixed_df.columns and 'event' in fixed_df.columns and 'time' in fixed_df.columns:
            ...
            fixed_df = fixed_df.drop_duplicates(subset=['swimmer', 'event'], keep='first')
        ...
        if 'event' in fixed_df.columns:
            ...
            for pattern, replace in replacements.items():
                fixed_df['event'] = fixed_df['event'].str.replace(pattern, replace, regex=True)
