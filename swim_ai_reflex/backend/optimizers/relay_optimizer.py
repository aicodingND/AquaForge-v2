...
class RelaySwimmer(BaseModel):
    ...
    exchange_skill: float = Field(default=1.0, ge=0, le=2, description="Exchange efficiency (1.0 = average)")
    pressure_factor: float = Field(default=1.0, ge=0, le=2, description="Performance under pressure")
...
class RelayOptimizer:
    ...
    def optimize_relay(
        ...
        if len(available_swimmers) < required_count:
            ...
            return RelayComposition(
                ...
                expected_time=sum(s.split_time for s in available_swimmers[:required_count]),
    ...
    def _evaluate_arrangement(
        ...
        lead_off = swimmers[0]
        leg1_time = lead_off.split_time + (lead_off.reaction_time - 0.70) * self.LEADOFF_REACTION_WEIGHT
        total_time += leg1_time
        rationale.append(f"Lead-off: {lead_off.name} (reaction: {lead_off.reaction_time:.2f}s)")
        ...
        for i, swimmer in enumerate(swimmers[1:3], 2):
            ...
            exchange_bonus = (swimmer.exchange_skill - 1.0) * -0.1  # Good exchange = negative adjustment
            leg_time += exchange_bonus
            total_time += leg_time
            rationale.append(f"Leg {i}: {swimmer.name} (exchange: {swimmer.exchange_skill:.1f})")
        ...
        rationale.append(f"Anchor: {anchor.name} (pressure factor: {anchor.pressure_factor:.1f})")
    ...
    def optimize_medley_relay(
        ...
        best_breast = min(breaststroke, key=lambda s: s.split_time) if breaststroke else None
