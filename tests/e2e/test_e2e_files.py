...
async def test_file_parsing():
    ...
    if seton_result["success"]:
        ...
        print(
            f"   Unique Swimmers: {df_seton['swimmer'].nunique() if 'swimmer' in df_seton.columns else 'N/A'}"
        )
    ...
    if immanuel_result["success"]:
        ...
        print(
            f"   Unique Swimmers: {df_imm['swimmer'].nunique() if 'swimmer' in df_imm.columns else 'N/A'}"
        )
    ...
    if seton_result["success"] and immanuel_result["success"]:
        ...
        print(
            f"   Immanuel (Boys filtered): {len(df_imm_filtered)} rows, {df_imm_filtered['swimmer'].nunique() if not df_imm_filtered.empty else 0} swimmers"
        )
        ...
        if not df_imm_filtered.empty:
            print(
                f"   Immanuel Events: {sorted(df_imm_filtered['event'].unique().tolist())}"
            )
