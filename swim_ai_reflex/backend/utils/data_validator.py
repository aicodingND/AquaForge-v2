...
def validate_roster_data(df: pd.DataFrame, meet_type: str = 'dual') -> Dict[str, List[str]]:
    """
    ...
    CHAMPIONSHIP_PATTERNS = [
        '13-14', '15-16', '17-18', 'Senior', 'Junior',
        '11-12', '8 & Under', '9-10', 'Age Group'
    ]
    ...
    if 'event' in df.columns:
        ...
        other_non_standard = [e for e in non_standard_events if e not in championship_events]
    ...
    if 'swimmer' in df.columns:
        empty_names = df[df['swimmer'].isna() | (df['swimmer'].str.strip() == '')].shape[0]
        if empty_names > 0:
            warnings.append(f"Found {empty_names} entries with missing swimmer names (these will be filtered out)")
