...
class DataFilterService(BaseService):
    ...
    def filter_for_dual_meet(
        ...
        if 'event' in filtered.columns:
            def normalize_event(raw_event: str) -> Optional[str]:
                if not raw_event:
                    return None
            ...
            self._log_filter("Event Normalization", "Standard 8 + Relays", pre_map_count, len(filtered))
        ...
        if 'event' in filtered.columns:
            ...
            self._log_filter("Event Type", f"Ind={include_individual}, Relay={include_relay}, Dive={include_diving}", pre_count, len(filtered))
        ...
        # Summary
        scoring_swimmers = filtered[filtered['scoring_eligible']]['swimmer'].nunique() if 'swimmer' in filtered.columns else 0
        non_scoring_swimmers = filtered[~filtered['scoring_eligible']]['swimmer'].nunique() if 'swimmer' in filtered.columns else 0
        ...
        print(f"Unique swimmers: {filtered['swimmer'].nunique() if 'swimmer' in filtered.columns else 'N/A'}")
        print(f"  - Scoring eligible (8-12): {scoring_swimmers}")
        print(f"  - Non-scoring (7th grade and below): {non_scoring_swimmers}")
        print(f"Events: {filtered['event'].nunique() if 'event' in filtered.columns else 'N/A'}")
        print(f"{'='*50}\n")
    ...
    def get_individual_events(self, df: pd.DataFrame) -> List[str]:
        ...
        return [e for e in events if 'relay' not in str(e).lower() and 'diving' not in str(e).lower()]
    
    def validate_for_dual_meet(self, seton_df: pd.DataFrame, opponent_df: pd.DataFrame) -> dict:
        """
