...
class CoachTendency(BaseModel):
    ...
    avoided_events: List[str] = Field(
        default_factory=list,
        description="Events coach tends to concede"
    )
    
    # Metadata
    sample_size: int = Field(default=0, ge=0, description="Number of meets analyzed")
    last_updated: datetime = Field(default_factory=datetime.now)
    confidence: float = Field(default=0.5, ge=0, le=1, description="Confidence in predictions")
...
class OpponentProfile(BaseModel):
    ...
    # Computed tendencies
    tendency: Optional[CoachTendency] = None
    
    # Performance stats
    win_rate_vs_seton: float = Field(default=0.5, ge=0, le=1)
    avg_margin: float = Field(default=0.0, description="Average point margin (+ = they win)")
    
    # Metadata
    last_meet_date: Optional[datetime] = None
    total_meets: int = 0
