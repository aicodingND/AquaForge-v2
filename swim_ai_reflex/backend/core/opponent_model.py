...
def greedy_opponent_best_lineup(opp_std: pd.DataFrame) -> pd.DataFrame:
    ...
    if opp_std is None or opp_std.empty:
        return pd.DataFrame(columns=['swimmer','grade','gender','event','time','team','is_relay'])
    ...
    result = pd.DataFrame(final_lineup) if final_lineup else pd.DataFrame(columns=df.columns)
