...
def validate_event_list(events: list[str], gender: str | None = None) -> dict[str, Any]:
    """
...
def validate_event_scoring(scored_df: pd.DataFrame, event: str) -> dict[str, Any]:
    ...
    if len(event_data) > 8:
        results["warnings"].append(f"More than 8 swimmers in event ({len(event_data)})")

    return results


def validate_meet_scoring(
    scored_df: pd.DataFrame, totals: dict[str, float], gender: str | None = None
) -> dict[str, Any]:
    """
    ...
    results["summary"] = {
        ...
        "percentage_of_max": (combined_total / MAX_TOTAL_POINTS * 100)
