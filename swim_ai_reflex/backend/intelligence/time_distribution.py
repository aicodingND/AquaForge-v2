...
def probability_of_beating(
    ...
    if swimmer1_std == 0 and swimmer2_std == 0:
        return 1.0 if swimmer1_mean < swimmer2_mean else (0.5 if swimmer1_mean == swimmer2_mean else 0.0)
...
def calculate_match_confidence(
    ...
    """
    prob = probability_of_beating(swimmer_mean, swimmer_std, opponent_mean, opponent_std)
...
def simulate_race(
    ...
    """
    results = {name: {i: 0 for i in range(1, len(swimmer_times) + 1)} 
