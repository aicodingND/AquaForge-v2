...
def validate_roster_dataframe(df: pd.DataFrame, required_columns: Optional[List[str]] = None) -> tuple[bool, List[str]]:
    """
    ...
    if 'grade' in df.columns:
        invalid_grades = df[~df['grade'].between(7, 12, inclusive='both')]['grade'].dropna()
        if not invalid_grades.empty:
            errors.append(f"Invalid grade values found: {invalid_grades.unique().tolist()}")
