...
class StrategyAnalysisService(BaseService):
    ...
    def analyze(
        ...
        return {
            ...
            "decisions": self._explain_all_decisions(
                seton_lineup, opponent_lineup, seton_roster
            ),
            "alternatives": self._generate_alternatives(
                seton_lineup, opponent_lineup, seton_roster, totals
            ),
        }
    ...
    def _generate_summary(
        ...
        for event in events:
            ...
            seton_pts = (
                seton_in_event["points"].sum()
                if "points" in seton_in_event.columns
                else 0
            )
            opp_pts = (
                opp_in_event["points"].sum() if "points" in opp_in_event.columns else 0
            )
        ...
        str_text = (
            f"Strong in: {', '.join(strengths[:3])}"
            if strengths
            else "No dominant events"
        )
        weak_text = (
            f"Vulnerable in: {', '.join(weaknesses[:3])}"
            if weaknesses
            else "No critical weaknesses"
        )
    ...
    def _explain_all_decisions(
        ...
        for swimmer in swimmers:
            ...
            if not full_roster.empty and "swimmer" in full_roster.columns:
                all_options = (
                    full_roster[full_roster["swimmer"] == swimmer]["event"]
                    .unique()
                    .tolist()
                )
            else:
            ...
            dropped = [
                e for e in all_options if e not in events_list and "Relay" not in e
            ]
            ...
            for _, row in swimmer_events.iterrows():
                ...
                opp_in_event = opponent_lineup[opponent_lineup["event"] == event]
                opp_best_place = (
                    opp_in_event["place"].min() if not opp_in_event.empty else 99
                )
            ...
            decisions.append(
                {
                    ...
                    "total_points": swimmer_events["points"].sum()
                    if "points" in swimmer_events.columns
                    else 0,
                }
            )
    ...
    def generate_exportable_report(
        self,
        summary: str,
        decisions: list[dict],
    ...
    def _generate_alternatives(
        ...
        alternatives = []
        original_margin = original_totals.get("seton", 0) - original_totals.get(
            "opponent", 0
        )
    ...
    def generate_event_suggestions(
        ...
        seton_in_lineup = (
            current_lineup[seton_mask].copy()
            if "team" in current_lineup.columns
            else current_lineup.copy()
        )

        # For each Seton swimmer, analyze their event options
        seton_swimmers = (
            seton_roster["swimmer"].unique()
            if "swimmer" in seton_roster.columns
            else []
        )
        ...
        for swimmer in seton_swimmers:
            ...
            current = (
                seton_in_lineup[seton_in_lineup["swimmer"] == swimmer]
                if "swimmer" in seton_in_lineup.columns
                else pd.DataFrame()
            )
            current_events = (
                current["event"].tolist()
                if not current.empty and "event" in current.columns
                else []
            )
            ...
            for _, row in swimmer_times.iterrows():
                ...
                opp_in_event = (
                    opponent_df[opponent_df["event"] == event]
                    if "event" in opponent_df.columns
                    else pd.DataFrame()
                )
                opp_times = (
                    sorted(opp_in_event["time"].tolist())
                    if not opp_in_event.empty and "time" in opp_in_event.columns
                    else []
                )
                ...
                for top_event in top_2:
                    if not top_event["in_lineup"]:
                        ...
                        if current_in_lineup:
                            worst_current = min(
                                current_in_lineup, key=lambda x: x["points"]
                            )
