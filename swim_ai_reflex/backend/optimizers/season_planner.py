...
class SeasonPlanner:
    ...
    def plan_season(
        self, season: Season, roster: pd.DataFrame, goals: SeasonGoals
    ) -> SeasonPlan:
        """
        ...
        for i, meet in enumerate(sorted_meets):
            ...
            for swimmer in strategy.get("jv_opportunities", []):
                jv_opportunities.append(
                    f"{swimmer} can swim {meet.opponent} ({meet.date})"
                )
    ...
    def _plan_single_meet(
        ...
        if (
            meet.importance == MeetImportance.REGULAR
            and goals.develop_jv_in_regular_meets
            and jv_swimmers
        ):
    ...
    def _calculate_championship_readiness(
        self, season: Season, stars: list[str], goals: SeasonGoals, swimmer_events: dict
    ) -> float:
    ...
    def get_next_meet_recommendation(
        self, season: Season, roster: pd.DataFrame
    ) -> dict:
        ...
        if next_meet.importance == MeetImportance.CHAMPIONSHIP:
            ...
            rec["recommendations"].append(
                f"Consider resting {stars[0] if stars else 'top swimmer'}"
            )
