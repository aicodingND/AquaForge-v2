"""
E2E Test: Seton vs Trinity - Girls Senior Events Only (No Diving, No Relays, No 13-14)
Simplified to only use Senior events for faster testing
"""
import sys
import os
import pandas as pd
...
def filter_girls_senior_events_only(df: pd.DataFrame) -> pd.DataFrame:
    """
    Filter DataFrame to include only girls SENIOR events (no diving, no relays, no 13-14).
    """
    ...
    # Filter out relays
    df_no_relays = df_girls[not df_girls['is_relay']].copy()
    print(f"[FILTER] After removing relays: {len(df_no_relays)} entries")
    
    # Filter out diving events
    df_no_diving = df_no_relays[~df_no_relays['event'].str.contains('Diving|Dives', case=False, na=False)].copy()
    print(f"[FILTER] After removing diving: {len(df_no_diving)} entries")
    
    # Filter for SENIOR events only (exclude 13-14 and 25 yard events)
    df_senior = df_no_diving[
        (df_no_diving['event'].str.contains('Senior', case=False, na=False)) &
        (~df_no_diving['event'].str.contains('25 ', case=False, na=False))
    ].copy()
    print(f"[FILTER] After keeping only Senior events (no 25 yard): {len(df_senior)} entries")
    
    # Show unique events
    unique_events = df_senior['event'].unique()
    print(f"[FILTER] Unique girls senior events found: {len(unique_events)}")
    for event in sorted(unique_events):
        count = len(df_senior[df_senior['event'] == event])
        print(f"  - {event}: {count} entries")
    
    return df_senior


async def run_e2e_girls_senior_only():
    """
    Run end-to-end test with Seton and Trinity PDFs, girls SENIOR events only.
    """
    print("\n" + "="*80)
    print("🏊‍♀️ E2E TEST: Seton vs Trinity - Girls SENIOR Events Only")
    print("(No Diving, No Relays, No 13-14, No 25 Yard)")
    print("="*80)
    ...
    # Filter for girls senior events only
    print("\n[STEP 2] 🔍 Filtering for Girls SENIOR Events Only...")
    print("\n--- SETON FILTERING ---")
    seton_girls = filter_girls_senior_events_only(seton_df)
    
    print("\n--- TRINITY FILTERING ---")
    trinity_girls = filter_girls_senior_events_only(trinity_df)
    
    # Check if we have data
    if len(seton_girls) == 0:
        print("\n[ERROR] ❌ No Seton girls senior events found after filtering!")
        return
    if len(trinity_girls) == 0:
        print("\n[ERROR] ❌ No Trinity girls senior events found after filtering!")
        return
    ...
    print(f"  Trinity: {trinity_swimmers} unique swimmers, {len(trinity_girls)} event entries")
    ...
    # Run optimization
    print("\n[STEP 3] 🚀 Running Optimization (Heuristic, 50 iterations)...")
    print("  Strategy: Best on Best (Trinity lineup will be optimized)")
    print("  Note: This may take 20-40 seconds...")
...
if __name__ == "__main__":
    asyncio.run(run_e2e_girls_senior_only())
