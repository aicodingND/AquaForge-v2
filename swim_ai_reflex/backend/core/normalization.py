...
def normalize_to_standard(normalized_df, team='seton'):
    if normalized_df is None or normalized_df.empty:
        return pd.DataFrame(columns=['swimmer','grade','gender','event','time','team','is_relay','is_diving','dive_score'])
    
    # Check if already standardized (from PDF parser)
    if 'swimmer' in normalized_df.columns and 'event' in normalized_df.columns and 'time' in normalized_df.columns:
        ...
        if 'event' in df.columns:
            df.loc[df['event'].astype(str).str.lower().str.contains('diving'), 'is_diving'] = True
        
        # Remove entries with empty swimmer names
        df = df[df['swimmer'].notna() & (df['swimmer'].astype(str).str.strip() != '')].reset_index(drop=True)
        ...
        if 'gender' in df.columns and df['gender'].notna().any():
            ...
            df.loc[df['gender'].str.startswith('M') | df['gender'].str.startswith('B'), 'gender'] = 'M'
            df.loc[df['gender'].str.startswith('F') | df['gender'].str.startswith('G'), 'gender'] = 'F'
        ...
        return df[ ['swimmer','grade','gender','event','time','team','is_relay','is_diving','dive_score'] ]
...
def load_roster_file(file_path, team_name):
    ...
    if ext == '.pdf':
        ...
        df, diag = load_file_dynamic(file_path)
        return normalize_to_standard(df, team=team_name)
...
def load_first_two_sheets_as_standard(uploaded_file, team_names=('seton','opponent')):
    ...
    def read_sheet_robust(sheet_name):
        ...
        for i, row in df_preview.iterrows():
            ...
            event_matches = sum(1 for x in row_str if any(k in x for k in ['free','back','breast','fly','im']))
            if event_matches >= 2:
                header_row_idx = i
                break
        
        # Read actual df with correct header
        df = pd.read_excel(uploaded_file, sheet_name=sheet_name, header=header_row_idx, dtype=object)
        
        # If the header row had a NaN/empty first column (common in this format), rename it to 'swimmer'
        if len(df.columns) > 0:
            first_col = df.columns[0]
            if str(first_col).startswith('Unnamed') or pd.isna(first_col) or str(first_col).strip() == '':
