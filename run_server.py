...
def run_reflex(port: int = 3000, backend_port: int = 8000):
    ...
    subprocess.run(
        ["reflex", "run", "--frontend-port", str(port), "--backend-port", str(backend_port)],
...
async def run_hybrid(
    ...
    reflex_process = subprocess.Popen(
        ["reflex", "run", "--frontend-port", str(reflex_port), "--backend-port", str(reflex_backend_port)],
        cwd=str(get_project_root()),
        env=env
    )
...
Examples:
  ...
  python run_server.py --mode api --port 8001 --reload  # Dev mode for API
    ...
    parser.add_argument(
        ...
        type=int,
        default=8001,
        help="FastAPI port (for hybrid mode)"
    )
    ...
    if args.mode == "api":
        ...
        asyncio.run(run_hybrid(
            reflex_port=args.port,
            fastapi_port=args.api_port
        ))
