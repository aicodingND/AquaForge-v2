...
async def test_optimization():
    ...
    df_seton_filt = data_filter_service.filter_for_dual_meet(
        df_seton, gender='M', include_individual=True, include_relay=False,
        include_diving=False, grades=[8,9,10,11,12], rules=rules
    )
    df_seton_filt['team'] = 'seton'
    
    df_imm_filt = data_filter_service.filter_for_dual_meet(
        df_immanuel, gender='M', include_individual=True, include_relay=False,
        include_diving=False, grades=[8,9,10,11,12], rules=rules
    )
    df_imm_filt['team'] = 'opponent'
    
    print(f"   Seton: {len(df_seton_filt)} entries, {df_seton_filt['swimmer'].nunique()} swimmers")
    print(f"   Immanuel: {len(df_imm_filt)} entries, {df_imm_filt['swimmer'].nunique()} swimmers")
    
    # Align meet data - returns tuple!
    print("\n3. Aligning meet data...")
    seton_aligned, opponent_aligned, alignment_info = align_meet_data(df_seton_filt, df_imm_filt)
    ...
    try:
        ...
        if response['success']:
            ...
            if lineup:
                ...
                seton_entries = [e for e in lineup if e.get('team', '').lower() == 'seton'][:4]
                opp_entries = [e for e in lineup if e.get('team', '').lower() != 'seton'][:4]
                
                print("   Seton:")
                for entry in seton_entries:
                    print(f"      {entry.get('event', 'N/A')}: {entry.get('swimmer', 'N/A')} - {entry.get('time', 'N/A')}s (pts: {entry.get('points', 0)})")
            ...
            if seton_score > 150 or opp_score > 150:
                print(f"   ⚠️ WARNING: Scores seem inflated ({seton_score}, {opp_score})")
            elif seton_score < 10 or opp_score < 10:
                print(f"   ⚠️ WARNING: Scores seem too low ({seton_score}, {opp_score})")
            else:
                print(f"   ✅ Scores look reasonable: Seton {seton_score} - Opponent {opp_score}")
                
            # Check unique swimmers per team
            df_lineup = pd.DataFrame(lineup)
            if not df_lineup.empty:
                seton_swimmers = df_lineup[df_lineup['team'].str.lower() == 'seton']['swimmer'].nunique()
                opp_swimmers = df_lineup[df_lineup['team'].str.lower() != 'seton']['swimmer'].nunique()
