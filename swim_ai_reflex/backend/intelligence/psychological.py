...
class PsychologicalProfile(BaseModel):
    ...
    strong_opponents: list[str] = Field(
        default_factory=list, description="Opponents they perform well against"
    )
    weak_opponents: list[str] = Field(
        default_factory=list, description="Opponents they struggle against"
    )

    # Recommendations
    best_meet_types: list[str] = Field(
        default_factory=list, description="Meet types where they excel"
    )
    avoid_pressure_events: bool = Field(
        default=False, description="Flag if swimmer struggles under pressure"
    )
...
class PsychologicalProfiler:
    ...
    def _analyze_clutch(
        self, times: list[dict], meet_lookup: dict[str, MeetResult]
    ) -> float:
    ...
    def _analyze_rivalry(
        self, times: list[dict], meet_results: list[MeetResult]
    ) -> float:
