...
from swim_ai_reflex.backend.core.normalization import (
...
class FileManager:
    ...
    def load_data(
        self, seton_path=None, opp_path=None, combined_path=None, filters=None
    ):
        seton_std = None
        opp_std = None
        ...
        try:
            if combined_path:
                seton_std, opp_std = load_first_two_sheets_as_standard(
                    combined_path, team_names=("seton", "opponent")
                )
            else:
                if seton_path:
