...
class StackelbergStrategy(BaseOptimizerStrategy):
    ...
    def optimize(
        ...
        **kwargs
        ...
        for i, star in enumerate(star_swimmers[:3]):
            variant = self._generate_lineup_resting(seton_df, events, rules, rest_swimmer=star)
        ...
        random_candidates = self._generate_random_lineups(
            seton_df, events, rules, 
            count=self.max_candidates - len(candidates)
        )
        ...
        for name, seton_lineup in candidates:
            try:
                ...
                # Score the matchup
                scored_df, totals = scoring_fn(pd.concat([seton_lineup, opp_response], ignore_index=True))
    ...
    def _generate_greedy_lineup(
        ...
        for event in events:
            event_swimmers = roster[roster['event'] == event].sort_values('time', ascending=True)
            assigned = 0
            max_per_event = 4 if "Relay" not in event else rules.max_relays_per_team_per_event
            ...
            for _, row in event_swimmers.iterrows():
                ...
                individual_count = sum(1 for e in swimmer_events.get(swimmer + "_events", []) if "Relay" not in e)
                
                if current_count >= rules.max_total_events_per_swimmer:
                    continue
                if is_individual and individual_count >= rules.max_individual_events_per_swimmer:
                    continue
                ...
                swimmer_events[swimmer + "_events"] = swimmer_events.get(swimmer + "_events", []) + [event]
                assigned += 1
        
        return pd.DataFrame(lineup_rows) if lineup_rows else pd.DataFrame(columns=roster.columns)
    
    def _identify_star_swimmers(self, roster: pd.DataFrame, top_n: int = 3) -> List[str]:
    ...
    def _compute_opponent_response(
        ...
        result = pd.DataFrame(lineup_rows) if lineup_rows else pd.DataFrame(columns=opponent_roster.columns)
