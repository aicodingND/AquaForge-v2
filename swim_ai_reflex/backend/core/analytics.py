...
def analyze_lineup(scored_lineup: List[Dict[str, Any]], meet_type: str) -> Dict[str, Any]:
    """
    ...
    swing_df = event_scores[event_scores['abs_diff'] <= SWING_THRESHOLD].sort_values('abs_diff')
    ...
    for event, row in swing_df.iterrows():
        insights['swing_events'].append({
            ...
            'diff': int(row['diff']),
            'status': 'Winning' if row['diff'] > 0 else 'Losing' if row['diff'] < 0 else 'Tied'
        })
    ...
    if 'points' in df.columns:
        ...
        first_place_wins = df[(df['team'] == 'seton') & (df['points'] >= FIRST_PLACE_PTS)]
        ...
        for event in df['event'].unique():
            ...
            if len(event_df) >= 2:
                ...
                if winner['team'] == 'opponent' and runner_up['team'] == 'seton':
                    desc = f"{runner_up['swimmer']} (Seton) vs {winner['swimmer']} (Opp)"
