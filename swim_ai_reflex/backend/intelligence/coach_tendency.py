...
class CoachTendencyAnalyzer:
    ...
    def analyze_history(
        ...
        relevant_meets = [
            m for m in past_meets if m.opponent_team.lower() == team_name.lower()
        ]
        ...
        confidence = min(
            1.0, len(relevant_meets) / 10.0
        )  # Full confidence at 10+ meets
    ...
    def _analyze_relay_resting(self, meets: list[MeetResult]) -> float:
        ...
        for meet in meets:
            ...
            stars = set()
            for entry in meet.opponent_lineup:
                if entry.get("place", 99) <= 2 and "Relay" not in entry.get(
                    "event", ""
                ):
    ...
    def _analyze_predictability(self, meets: list[MeetResult]) -> float:
        ...
        for i in range(1, len(meets)):
            prev_lineup = {
                (e.get("swimmer"), e.get("event")) for e in meets[i - 1].opponent_lineup
            }
            curr_lineup = {
                (e.get("swimmer"), e.get("event")) for e in meets[i].opponent_lineup
            }
    ...
    def _analyze_adaptation(self, meets: list[MeetResult]) -> float:
        ...
        lineups = []
        for meet in meets:
            lineup_sig = frozenset(
                (e.get("swimmer"), e.get("event")) for e in meet.opponent_lineup
            )
    ...
    def predict_lineup(
        ...
        our_roster: pd.DataFrame | None = None,
    ) -> pd.DataFrame:
        """
        ...
        if coach.predictable_star_placement > 0.6 and coach.favorite_events_for_stars:
            events = sorted(
                events, key=lambda e: 0 if e in coach.favorite_events_for_stars else 1
            )
        ...
        for event in events:
            ...
            if "Relay" in event and coach.rests_stars_in_relays > 0.6:
                ...
                event_swimmers = (
                    event_swimmers.iloc[2:]
                    if len(event_swimmers) > 2
                    else event_swimmers
                )
            ...
            if event in coach.avoided_events:
                ...
                event_swimmers = (
                    event_swimmers.iloc[1:]
                    if len(event_swimmers) > 1
                    else event_swimmers
                )
