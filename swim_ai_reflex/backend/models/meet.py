...
class ScheduledMeet(BaseModel):
    ...
    importance_weight: float = Field(
        ...
        description="Weight multiplier for this meet"
    )
    ...
    # Historical context
    last_result: Optional[str] = None  # "W 105-75" or "L 85-95"
    historical_margin: float = Field(default=0.0, description="Avg point margin vs this opponent")
...
class Season(BaseModel):
    ...
    championship_meet: Optional[ScheduledMeet] = None
    ...
    def upcoming_meets(self) -> List[ScheduledMeet]:
        ...
        today = date.today()
        return sorted(
            [m for m in self.meets if m.date >= today],
            key=lambda m: m.date
        )
    
    @property
    def next_meet(self) -> Optional[ScheduledMeet]:
        """Get the next upcoming meet."""
        upcoming = self.upcoming_meets
        return upcoming[0] if upcoming else None
    
    @property
    def days_until_championship(self) -> Optional[int]:
...
class SeasonGoals(BaseModel):
    ...
    swimmer_event_goals: dict = Field(
        ...
        description="e.g., {'John': {'100 Free': 'break 50s'}}"
    )
...
class SeasonPlan(BaseModel):
    ...
    swimmer_load: dict = Field(
        default_factory=dict,
        description="Events per swimmer across season"
    )
