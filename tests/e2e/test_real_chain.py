...
async def test_full_chain():
    ...
    df_seton = data_filter_service.filter_for_dual_meet(
        seton_res["data"], gender="M", grades=[8, 9, 10, 11, 12], rules=rules
    )
    df_seton["team"] = "seton"
    df_imm = data_filter_service.filter_for_dual_meet(
        imm_res["data"], gender="M", grades=[8, 9, 10, 11, 12], rules=rules
    )
