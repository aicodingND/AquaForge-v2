...
class MeetRepository(ABC):
    ...
    @abstractmethod
    async def get_meet(self, meet_id: str) -> MeetResult | None:
    ...
    @abstractmethod
    async def get_tendency(self, team_name: str) -> CoachTendency | None:
    ...
    @abstractmethod
    async def get_opponent(self, team_name: str) -> OpponentProfile | None:
...
class InMemoryRepository(MeetRepository):
    ...
    async def get_meets_by_opponent(self, opponent: str) -> list[MeetResult]:
        return [
            m
            for m in self._meets.values()
            if m.opponent_team.lower() == opponent.lower()
        ]
    ...
    async def get_meets_in_range(
        ...
        return [m for m in self._meets.values() if start_date <= m.date <= end_date]
...
_repository: MeetRepository | None = None
