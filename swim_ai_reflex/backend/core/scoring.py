...
EVENT_ORDER = [
    ...
    for variant in [f"Girls {ev}", f"Boys {ev}", ev]
]
...
def score_event_with_rules(df_event: pd.DataFrame, rules: MeetRules) -> pd.DataFrame:
    ...
    """
    if df_event.empty:
        return df_event.assign(place=pd.Series(dtype=int), points=0.0, scoring_eligible=False)

    is_diving = df_event['is_diving'].any() if 'is_diving' in df_event.columns else False
    
    # Sort
    if is_diving:
        # Descending for diving score
        # Ensure 'dive_score' is used if available, else 'time'
        col = 'dive_score' if 'dive_score' in df_event.columns and df_event['dive_score'].notnull().any() else 'time'
    ...
    if 'grade' in df_sorted.columns:
        df_sorted['scoring_eligible'] = df_sorted['grade'].apply(lambda g: is_scoring_grade(g, min_grade=rules.min_scoring_grade))
    else:
    ...
    if 'penalty' in scoring_entries.columns:
        scoring_entries['points'] = scoring_entries['points'] - scoring_entries['penalty'].fillna(0)
    
    # Enforce Max Scorers Per Team (e.g. only top 3 score)
    if 'team' in scoring_entries.columns:
        scorer_limit = rules.max_scorers_per_team_relay if is_relay else rules.max_scorers_per_team_individual
        
        team_counts = {}
        for idx, row in scoring_entries.iterrows():
            tm = normalize_team_name(row['team'])
            current_count = team_counts.get(tm, 0)
            
            if current_count >= scorer_limit:
                scoring_entries.at[idx, 'points'] = 0.0
            elif scoring_entries.at[idx, 'points'] > 0:
                 team_counts[tm] = current_count + 1
    
    # Handle non-scoring entries (Assign 0 points and 'Exh' place)
    non_scoring_entries['place'] = 999  # Or 'Exh' if string allowed, but keeping int for safety
    ...
    sort_col = 'dive_score' if is_diving and 'dive_score' in df_result.columns else 'time'
...
def apply_event_entry_limits(df: pd.DataFrame, rules: MeetRules) -> pd.DataFrame:
    ...
    for ev, grp in df.groupby('event'):
        ...
        for team_norm in grp['team_norm'].unique():
            ...
            # Sort best first
            sort_col = 'dive_score' if is_diving and 'dive_score' in team_rows.columns else 'time'
...
def full_meet_scoring(long_roster: pd.DataFrame, rules: MeetRules = None, validate: bool = True) -> Tuple[pd.DataFrame, Dict[str, float]]:
    if rules is None:
        rules = VISAADualRules()

    if long_roster is None or long_roster.empty:
        cols = ['swimmer','grade','event','time','team','is_relay','place','points']
    ...
    if 'is_relay' not in long_roster.columns:
        if 'event' in long_roster.columns:
            long_roster['is_relay'] = long_roster['event'].str.lower().str.contains('relay', na=False)
        else:
            long_roster['is_relay'] = False
    if 'is_diving' not in long_roster.columns:
        if 'event' in long_roster.columns:
            long_roster['is_diving'] = long_roster['event'].str.lower().str.contains('diving|dive', regex=True, na=False)
        else:
    ...
    totals = {
        'seton': float(full_scored.loc[full_scored['team_norm'] == 'seton', 'points'].sum()),
        'opponent': float(full_scored.loc[full_scored['team_norm'] == 'opponent', 'points'].sum())
    }
    
    # 4. VALIDATION: Check if scoring follows standard dual meet rules
    if validate:
        try:
            from swim_ai_reflex.backend.core.scoring_validator import validate_meet_scoring, print_validation_report
            ...
            if 'gender' in full_scored.columns:
                ...
                if len(genders) == 1:
                    gender = 'Girls' if genders[0] == 'F' else 'Boys' if genders[0] == 'M' else None
