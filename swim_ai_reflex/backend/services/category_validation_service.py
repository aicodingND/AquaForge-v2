...
def analyze_dataset_categories(df: pd.DataFrame, team_name: str = "Team") -> dict:
    ...
    logger.info(
        f"  Individual: {analysis['has_individual']} ({len(individual_events)} events)"
    )
    ...
    logger.info(
        f"  Total: {len(all_events)} events, {len(df)} entries, {swimmer_count} swimmers"
    )
...
def find_common_categories(seton_analysis: dict, opponent_analysis: dict) -> dict:
    ...
    common_individual = (
        seton_analysis["has_individual"] and opponent_analysis["has_individual"]
    )
    ...
    if not common_boys and (
        seton_analysis["has_boys"] or opponent_analysis["has_boys"]
    ):
        if seton_analysis["has_boys"] and not opponent_analysis["has_boys"]:
            ...
            recommendations.append(
                "Remove boys events from Seton data or add to opponent"
            )
        else:
            warnings.append("Opponent has boys events but Seton doesn't")
            recommendations.append(
                "Remove boys events from opponent data or add to Seton"
            )

    if not common_girls and (
        seton_analysis["has_girls"] or opponent_analysis["has_girls"]
    ):
        if seton_analysis["has_girls"] and not opponent_analysis["has_girls"]:
            ...
            recommendations.append(
                "Remove girls events from Seton data or add to opponent"
            )
        else:
            warnings.append("Opponent has girls events but Seton doesn't")
            recommendations.append(
                "Remove girls events from opponent data or add to Seton"
            )

    if not common_relays and (
        seton_analysis["has_relays"] or opponent_analysis["has_relays"]
    ):
        warnings.append("Relay events not present in both teams")
        recommendations.append(
            "Ensure both teams have relay data or remove relays from both"
        )

    if not common_diving and (
        seton_analysis["has_diving"] or opponent_analysis["has_diving"]
    ):
        warnings.append("Diving events not present in both teams")
        recommendations.append(
            "Ensure both teams have diving data or remove diving from both"
        )

    if (
        len(common_events) < len(seton_events) * 0.5
        or len(common_events) < len(opponent_events) * 0.5
    ):
    ...
    if seton_events or opponent_events:
        alignment_score = (
            len(common_events) / max(len(seton_events), len(opponent_events))
        ) * 100
    else:
        alignment_score = 0
...
def filter_to_common_categories(
    seton_df: pd.DataFrame, opponent_df: pd.DataFrame, common_analysis: dict
