...
async def compare_teams(
    seton_data: list[dict[str, Any]], opponent_data: list[dict[str, Any]]
):
    """
    ...
    try:
            ...
            event_analysis.append(
                {
                    ...
                    "advantage": "seton"
                    if len(seton_entries) > len(opponent_entries)
                    else (
                        "opponent"
                        if len(opponent_entries) > len(seton_entries)
                        else "even"
                    ),
                }
            )
        ...
        recommendations = []
        seton_strong_events = [
            e["event"] for e in event_analysis if e["advantage"] == "seton"
        ]
        opponent_strong_events = [
            e["event"] for e in event_analysis if e["advantage"] == "opponent"
        ]

        if seton_strong_events:
            recommendations.append(
                f"Seton has depth advantage in: {', '.join(seton_strong_events[:3])}"
            )
        if opponent_strong_events:
            recommendations.append(
                f"Focus training on: {', '.join(opponent_strong_events[:3])}"
            )
...
async def analyze_swimmer(swimmer_name: str, team_data: list[dict[str, Any]]):
    """
    ...
    try:
        ...
        if not swimmer_entries:
            raise HTTPException(
                status_code=404, detail=f"Swimmer '{swimmer_name}' not found"
            )
...
async def analyze_event(
    ...
    try:
        ...
        return {
            ...
            "depth_advantage": "seton"
            if len(seton_swimmers) > len(opponent_swimmers)
            else "opponent",
        }
...
async def analyze_depth(team_data: list[dict[str, Any]]):
    ...
    try:
        ...
        versatile_swimmers = [
            name for name, events in swimmer_events.items() if len(events) >= 3
        ]
...
async def get_scoring_rules():
    ...
    return {
        ...
        "relay_events": {"1st": 8, "2nd": 4, "3rd": 0},
        "notes": [
