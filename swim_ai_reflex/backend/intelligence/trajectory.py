...
class SwimmerTrajectory(BaseModel):
    ...
    improvement_rate: float = Field(
        default=0.0, 
        description="Seconds per month improvement (negative = faster)"
    )
    ...
    plateau_start: Optional[date] = None
...
class TrajectoryPredictor:
    ...
    def analyze_trajectory(
        ...
        data_points = [
            TrajectoryPoint(date=t.meet_date, time=t.time)
        ...
        predictions = self._predict_future(
            sorted_times, grade, months_ahead=3
        )
    ...
    def _predict_future(
        ...
        for month in range(1, months_ahead + 1):
            ...
            predictions.append(TrajectoryPoint(
                date=future_date,
