...
def validate_roster_data(
    ...
    if "gender" not in df.columns:
        warnings.append(
            "! 'Gender' column not found. Please ensure your file has a column named 'Gender' or 'Sex' to correctly separate Boys/Girls events."
        )
    ...
    if "event" in df.columns:
        ...
        if championship_events:
            warnings.append(
                f"! Data appears to be from a CHAMPIONSHIP or INVITATIONAL meet (found age group events: {', '.join(championship_events[:3])}). "
    ...
    if "gender" in df.columns:
        ...
        if len(genders) > 1:
            warnings.append(
                f"! Mixed genders detected ({', '.join([str(g) for g in genders])}). "
    ...
    if "grade" in df.columns:
        ...
        if missing_grades > 5:
            warnings.append(
                f"! {missing_grades} swimmers missing grade information. "
    ...
    if "swimmer" in df.columns:
        ...
        if num_swimmers < 8:
            warnings.append(
                f"! Only {num_swimmers} unique swimmers found. "
    ...
    if len(df) > 500:
        warnings.append(
            f"! Dataset contains {len(df)} entries (very large). "
