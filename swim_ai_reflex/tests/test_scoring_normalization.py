import pandas as pd
from swim_ai_reflex.backend.core.scoring import score_event_with_rules, full_meet_scoring
...
class TestScoring:
    def test_score_event_mixed_case_teams(self):
        ...
        df = pd.DataFrame([
            {'swimmer': 'A', 'time': 20.0, 'team': 'Seton', 'is_relay': False, 'is_diving': False}, # 1st, 6 pts
            {'swimmer': 'B', 'time': 21.0, 'team': 'SETON', 'is_relay': False, 'is_diving': False}, # 2nd, 4 pts
            {'swimmer': 'C', 'time': 22.0, 'team': 'Seton ', 'is_relay': False, 'is_diving': False}, # 3rd, 3 pts
            {'swimmer': 'D', 'time': 23.0, 'team': 'seton', 'is_relay': False, 'is_diving': False}, # 4th, 2 pts -> 0 pts (Max 3)
            {'swimmer': 'E', 'time': 24.0, 'team': 'Opponent', 'is_relay': False, 'is_diving': False}, # 5th, 1 pt
        ])
    ...
    def test_full_meet_scoring_totals(self):
        ...
        df = pd.DataFrame([
            {'event': '50 Free', 'swimmer': 'A', 'time': 20.0, 'team': 'Seton', 'is_relay': False, 'is_diving': False},
            {'event': '50 Free', 'swimmer': 'B', 'time': 21.0, 'team': 'Opponent', 'is_relay': False, 'is_diving': False},
        ])
